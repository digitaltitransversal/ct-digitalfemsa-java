java:
	...
	docker run --rm \
		...
		-i https://raw.githubusercontent.com/digitaltitransversal/openapi/refs/heads/feature/BOPR-2819-checkout-param/_build/api.yaml \
		-g java \
		-o /local \
...
version-update:
	...
	@perl -i -pe 'if (!$$done && s|<version>[^<]*</version>|<version>$(NEW_VERSION)</version>|) { $$done = 1 }' pom.xml
	...
	@sed -i '' 's/userAgentFields.put("sdk_version", "[^"]*");/userAgentFields.put("sdk_version", "$(NEW_VERSION)");/' src/main/java/io/digitalfemsa/ApiClient.java
